...
function Theatre() {
  ...
  const getCurrentScript = () => {
    if (!actOneScripts.scenes || !actOneScripts.scenes[currentScene]) {
      return "The quick brown fox jumps over the lazy dog.";
    }
    const scene = actOneScripts.scenes[currentScene];
    if (!scene.exercises || !scene.exercises[currentExercise]) {
      return scene.warmUp || "The quick brown fox jumps over the lazy dog.";
    }
    const exercise = scene.exercises[currentExercise];
    return exercise.script || scene.warmUp;
  };

  const handleSceneComplete = (performance) => {
    // Add defensive checks for scene and exercise
    if (!actOneScripts.scenes || !actOneScripts.scenes[currentScene]) {
  ...
  const scene = actOneScripts.scenes ? actOneScripts.scenes[currentScene] : null;
  const exercise = scene?.exercises ? scene.exercises[currentExercise] : null;
    ...
    <div className="min-h-screen">
      <div className="theatre-container">
        <TheatricalHeader
          ...
          exerciseTitle={exercise?.title || "Preparing..."}
        />
        ...
        <MainStage
          ...
          sceneInfo={exercise || { tips: "Let's begin your performance!" }}
